...
class TimerSubscriberChannel extends FDisposableBase implements FChannelSubscriber<Date> {
	...
	private onTimer() {
		const now = new Date();
		this._handlers.forEach(h => h(FExecutionContext.Empty, { data: now }));
	}
}
...
async function main() {
	...
	server1 = new THE.SecuredWebServer({
		...
		clientCertificateMode: FHostingConfiguration.ClientCertificateMode.XFCC,
	...
	await restEndpoint.init(FExecutionContext.Empty);
	//await wsEndpoint.init(FExecutionContext.Empty);

	await server1.init(FExecutionContext.Empty);
	await server2.init(FExecutionContext.Empty);
